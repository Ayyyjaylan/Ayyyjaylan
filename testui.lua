...
if type(startup_args) ~= "table" then
    startup_args = {}
end
...
do
    ...
    for i, v in pairs(startup_args) do
        library[i] = v
    end
...
do
    ...
    function library:notification(message, duration, color)
        ...
        function notification:remove()
            ...
            task.spawn(function()
                utility:tween(notification.objects.container, 'Position', udim2_new(0, 5, 0, -100), 0.3).Completed:Wait()
                notification.objects.container:Destroy()
            end)
        ...
        delay(0.15 + (duration or 5) + 0.15, function()
            notification:remove()
        end)
    ...
    function utility:tween(object, property, value_new, time, style, direction)
        ...
        tween.connection = library:connection(runservice.Heartbeat, function(delta)
            ...
            if utility.table.includes(object, '_object') and (object._object and type(object._object) == 'table' and rawget(object._object, '__OBJECT_EXISTS') ~= true) then
                tween.connection:Disconnect()
    ...
    function utility.table.getdescendants(tbl, new)
        ...
        for i, v in next, tbl do
            -- Check if 'v' is a table and not a Roblox instance like DrawingObject
            if typeof(v) == "table" and not pcall(function() return v:IsA("Instance") end) then
                utility.table.getdescendants(v, new)
            elseif not pcall(function() return v:IsA("Instance") end) then
                new[i] = v
            end
        end
    ...
    function utility.table.deepcopy(tbl)
        ...
        for i, v in next, tbl or {} do
            -- Check if 'v' is a table and not a Roblox instance
            if typeof(v) == 'table' and not pcall(function() return v:IsA("Instance") end) then
    ...
    function utility.table.clearkeys(tbl)
        local new = {}
        for i, v in next, tbl do
            -- Only add keys that are not instances (i.e., non-DrawingObject types)
            if not pcall(function() return v:IsA("Instance") end) then
                table.insert(new, i)
            end
        end
        return new
...
do
    ...
    library:define('drawing', function(_, class, properties, readonly)
        ...
        function drawing:Remove()
            ...
            if drawing._object and typeof(drawing._object) == "Instance" and drawing._object:IsA("DrawingObject") then
